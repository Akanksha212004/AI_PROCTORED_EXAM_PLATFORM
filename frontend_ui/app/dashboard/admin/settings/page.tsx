  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1.5 text-sm text-paper/60">{t("admin.settings.subtitle")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        <Card>
          <form onSubmit={handleProfileSubmit} className="space-y-4">
            <p className="flex items-center gap-2 font-display text-base font-semibold text-paper">
              <UserIcon className="h-4 w-4 text-accent-sky" />
              {t("admin.settings.profile.heading")}
            </p>

            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">
                {t("admin.settings.profile.name")}
              </label>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">
                {t("admin.settings.profile.email")}
              </label>
              <input value={user?.email ?? ""} disabled className={inputClass} />
              <p className="mt-1 text-xs text-muted">{t("admin.settings.profile.emailHint")}</p>
            </div>

            <Button type="submit" className="w-auto px-4" disabled={isSavingProfile || name.trim().length < 2}>
              {isSavingProfile ? t("admin.settings.profile.saving") : t("admin.settings.profile.save")}
            </Button>
          </form>
        </Card>
        ...
        <Card>
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted">
                {t("admin.settings.password.current")}
              </label>
              <input
                type="password"
                value={currentPassword}
            ...
            <div>
              ...
              <input
                ...
                minLength={8}
              />
              <p className="mt-1 text-xs text-muted">{t("admin.settings.password.newHint")}</p>
            </div>
            ...
            <Button
              ...
              disabled={isSavingPassword || !currentPassword || !newPassword}
            >
              {isSavingPassword ? t("admin.settings.password.submitting") : t("admin.settings.password.submit")}
            </Button>
          </form>
        </Card>
      </div>
