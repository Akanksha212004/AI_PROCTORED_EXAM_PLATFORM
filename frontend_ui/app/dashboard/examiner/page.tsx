...
// import { ProctorAlertsCard } from "@/components/dashboard/ProctorAlertsCard";
...
//       {/* Quick actions */}
//       <section>
//         <p className="mb-4 text-xs font-medium uppercase tracking-wide text-muted">Quick Actions</p>
//         <QuickActionsRow />
//       </section>

//       {/* Main grid: exam/submission activity + proctoring & schedule */}
...
function ExaminerDashboardContent() {
  ...
  return (
    <div className="space-y-10">
      {/* Hero */}
      <div className="relative -mx-6 overflow-hidden rounded-2xl bg-grid-fade px-6 pb-6 pt-2 sm:-mx-8 sm:px-8">
        <div className="flex flex-col gap-5 border-b border-border pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="mb-1 flex items-center gap-1.5 font-mono text-xs uppercase tracking-[0.2em] text-accent-sky">
              <LayoutDashboard className="h-3.5 w-3.5" />
      ...
      {/* Main grid: exam & submission activity + schedule */}
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.4fr_1fr]">
        <div className="space-y-5">
          <ExamOverviewCard />
          <RecentSubmissionsCard />
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {QUICK_ACCESS.map((item) => (
            <Link key={item.href} href={item.href}>
              <Card
                interactive
                className={cn("flex h-full items-center gap-4", TONE_HOVER_BORDER[item.tone])}
              >
                <div className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-lg", TONE_ICON_BG[item.tone])}>
                  {item.icon}
                </div>
