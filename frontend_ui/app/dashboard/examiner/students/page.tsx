...
function StudentsContent() {
  ...
  // Local override so the badge/button reflect a status change immediately,
  // without waiting on a full refetch of the list.
  ...
  const resolvedItems = items.map((s) => ({
    ...s,
    isActive: statusOverrides[s.id] ?? s.isActive,
  }));
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full min-w-[820px] text-left text-sm">
              ...
              <tbody>
                {resolvedItems.map((student) => (
                  <StudentRow key={student.id} student={student} onToggleStatus={setStatusTarget} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      <ToggleStudentStatusDialog
        student={statusTarget}
        onClose={() => setStatusTarget(null)}
        onUpdated={(studentId, isActive) => {
          setStatusOverrides((prev) => ({ ...prev, [studentId]: isActive }));
        }}
      />
    </div>
  );
