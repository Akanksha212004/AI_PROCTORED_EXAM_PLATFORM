  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-paper">{t("admin.exams.title")}</h1>
          <p className="text-sm text-muted">{t("admin.exams.subtitle")}</p>
        </div>
        <Button onClick={() => setFormState({ open: true, mode: "create", exam: null })} className="w-full px-4 sm:w-auto">
          <Plus className="h-4 w-4" />
          {t("admin.exams.newExam")}
        </Button>
      </div>
      ...
      <Card className="p-5">
        ...
        />
        <Pagination pagination={pagination} onPageChange={setPage} itemLabelKey="common.itemLabels.exams" />
      </Card>
