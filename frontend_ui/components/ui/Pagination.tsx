...
interface Props {
  pagination: QuestionPagination;
  onPageChange: (page: number) => void;
  /** Translation key for the noun shown after the count, e.g. "common.itemLabels.exams". Defaults to "common.itemLabels.questions". */
  itemLabelKey?: string;
}

export function Pagination({ pagination, onPageChange, itemLabelKey = "common.itemLabels.questions" }: Props) {
  const { t } = useI18n();
  const { page, totalPages, total, limit } = pagination;
  if (total === 0) return null;
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-3 border-t border-border px-1 py-4 sm:flex-row">
      <p className="text-sm text-muted">
        {t("common.pagination.showing")} <span className="text-paper">{start}</span>–
        <span className="text-paper">{end}</span> {t("common.pagination.of")}{" "}
        <span className="text-paper">{total}</span> {t(itemLabelKey)}
      </p>
      <div className="flex items-center gap-2">
        <button
          onClick={() => onPageChange(page - 1)}
        ...
        <span className="text-sm text-muted">
          {t("common.pagination.page")} <span className="text-paper">{page}</span> {t("common.pagination.of")}{" "}
