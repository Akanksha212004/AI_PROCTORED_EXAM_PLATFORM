...
const STATUS_LABEL_KEY: Record<ExaminerApprovalStatus, string> = {
  PENDING: "admin.examinerRequests.statusPending",
  APPROVED: "admin.examinerRequests.statusApproved",
  REJECTED: "admin.examinerRequests.statusRejected",
};
  ...
  if (requests.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <p className="text-sm text-muted">{t("admin.examinerRequests.table.empty")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[900px] text-left text-sm">
        <thead>
          <tr className="border-b border-border text-xs uppercase tracking-wide text-muted">
            <th className="px-5 py-3 font-medium">{t("admin.examinerRequests.table.applicant")}</th>
            <th className="px-5 py-3 font-medium">{t("admin.examinerRequests.table.institutionDepartment")}</th>
            <th className="px-5 py-3 font-medium">{t("admin.examinerRequests.table.designation")}</th>
            <th className="px-5 py-3 font-medium">{t("admin.examinerRequests.table.requested")}</th>
            <th className="px-5 py-3 font-medium">{t("admin.examinerRequests.table.status")}</th>
            <th className="px-5 py-3 font-medium text-right">{t("admin.examinerRequests.table.actions")}</th>
          </tr>
        </thead>
        <tbody>
          {requests.map((request) => (
              ...
              <td className="px-5 py-3.5">
                ...
                {request.employeeId && (
                  <p className="text-xs text-paper/40">
                    {t("admin.examinerRequests.table.idPrefix", { id: request.employeeId })}
                  </p>
                )}
              </td>
              <td className="px-5 py-3.5 text-paper/80">
              ...
              <td className="px-5 py-3.5 text-paper/80">
                ...
                {request.yearsOfExperience !== null && (
                  <p className="text-xs text-muted">
                    {t("admin.examinerRequests.table.yearsExperience", { count: request.yearsOfExperience })}
                  </p>
                )}
              </td>
              <td className="px-5 py-3.5 font-mono text-xs text-muted">
                {relativeTime(request.createdAt, t, language)}
              </td>
              <td className="px-5 py-3.5">
                <Badge tone={STATUS_TONE[request.approvalStatus]}>{t(STATUS_LABEL_KEY[request.approvalStatus])}</Badge>
              </td>
              <td className="px-5 py-3.5">
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="rounded-lg border border-accent-rose/30 px-3 py-1.5 text-xs font-medium text-accent-rose transition-colors hover:bg-accent-rose/10 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    {t("admin.examinerRequests.table.reject")}
                  </button>
                  <button
                    onClick={() => onApprove(request)}
                    disabled={request.approvalStatus !== "PENDING"}
                    className="rounded-lg border border-accent-teal/30 px-3 py-1.5 text-xs font-medium text-accent-teal transition-colors hover:bg-accent-teal/10 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    {t("admin.examinerRequests.table.approve")}
                  </button>
                </div>
              </td>
            </tr>
