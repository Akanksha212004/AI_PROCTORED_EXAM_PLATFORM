...
function ObjectiveAnswerView({ q }: { q: SubmissionQuestionDetail }) {
  // Dynamic (examiner-authored) content — option text — machine
  // translated the same way as everywhere else. Self-contained hook
  // call here (rather than threading translated strings down from the
  // parent) keeps this component's props unchanged.
  const optionTexts = q.options?.map((o) => o.optionText) ?? [];
  const { translated } = useTranslatedTexts(optionTexts);
...
export function GradingDialog({ sessionId, onClose, onGraded }: Props) {
  ...
  // Dynamic (examiner-authored) content — exam title + each question's
  // text. Deliberately NOT translated: q.submittedText (the student's
  // own typed answer — an examiner grading it needs to read exactly
  // what was written, not a machine paraphrase) and grading.feedback
  // (an editable input the examiner is actively typing into).
  const dynamicTexts = detail
    ? [detail.examTitle, ...detail.questions.map((q) => q.questionText)]
    : [];
  const { translated: translatedDynamicTexts } = useTranslatedTexts(dynamicTexts);
  const translatedExamTitle = detail ? translatedDynamicTexts[0] ?? detail.examTitle : "";
        ...
        {isLoading ? (
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-4">
              <div>
                <p className="text-sm text-paper">{translatedExamTitle}</p>
              ...
              {detail.questions.map((q, i) => (
                <div key={q.questionId} className="space-y-2">
                  <p className="text-sm font-medium text-paper">
                    Q{i + 1}. {translatedDynamicTexts[i + 1] ?? q.questionText}{" "}
